...
/* O filtro atualiza a cada N+1 amostras (so' utilizar potencias de 2) */
#define N 3
...
typedef struct {
 ...
 /* Valor do limiar de convergência */
...
connectPortToSimpleAdaptiveFilter(LADSPA_Handle Instance,
  ...
  case SDL_MU:
    ...
    break;
  case SDL_THRESHOLD:
    psSimpleAdaptiveFilter->m_pfThreshold = DataLocation;
    break;
  case SDL_INPUTD:
...
runSimpleAdaptiveFilter(LADSPA_Handle Instance,
  ...
  unsigned short lNcount = 0; /* Contador da convolucao */
  ...
  fThreshold = *psSimpleAdaptiveFilter->m_pfThreshold;
...
_init() {
  ...
  if (g_psDescriptor) {
    ...
    g_psDescriptor->Name
      = strdup("Simple LMS SIGN atualizacao a cada 4 amostras e limiar de convergencia");
    ...
    piPortDescriptors[SDL_MU]
      = LADSPA_PORT_INPUT | LADSPA_PORT_CONTROL;
    piPortDescriptors[SDL_THRESHOLD]
      = LADSPA_PORT_INPUT | LADSPA_PORT_CONTROL;
    ...
    pcPortNames[SDL_THRESHOLD]
      = strdup("Th - Limiar de convergencia");
    ...
    psPortRangeHints[SDL_MU].UpperBound
      = 0.05;
    psPortRangeHints[SDL_THRESHOLD].HintDescriptor
      = (LADSPA_HINT_BOUNDED_BELOW
	 | LADSPA_HINT_BOUNDED_ABOVE
	 | LADSPA_HINT_DEFAULT_LOW
	 | LADSPA_HINT_LOGARITHMIC);
    psPortRangeHints[SDL_THRESHOLD].LowerBound
      = 0;
    psPortRangeHints[SDL_THRESHOLD].UpperBound
      = 1;
